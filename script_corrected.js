...
const BLOCKS_A = [
  { minTrials: 20, maxTrials: 30, rotation:  0, lag:   0 },
  { minTrials: 30, maxTrials: 40, rotation: 30, lag:   0 },
  { minTrials: 30, maxTrials: 40, rotation: 30, lag: 250 },
  { minTrials: 20, maxTrials: 30, rotation:  0, lag:   0 },
];

const BLOCKS_B = [
  { minTrials: 20, maxTrials: 30, rotation:  0, lag:   0 },
  { minTrials: 30, maxTrials: 40, rotation:  0, lag: 250 },
  { minTrials: 30, maxTrials: 40, rotation: 30, lag: 250 },
  { minTrials: 20, maxTrials: 30, rotation:  0, lag:   0 },
];
...
window.onload = function() {
  ...
  function calculateSpeedProfile(path) {
  ...
  function normalizeArray(arr) {
    ...
    return arr.map(v => (v - mean) / stdDev);
  }

  function computeSpectrum(signal) {
    const N = signal.length;
    if (N < 4) return { spectrum: [1], frequencies: [0] };
    const spectrum = [];
    const frequencies = [];
    ...
    for (let k = 0; k < maxK; k++) {
        ...
        spectrum.push(Math.sqrt(realSum * realSum + imagSum * imagSum) / N);
        frequencies.push(k);
    }
    return { spectrum, frequencies };
  }

  function calculateSPARC(speeds) {
    if (speeds.length < 5) return -1;
    const normalized = normalizeArray(speeds);
    const { spectrum, frequencies } = computeSpectrum(normalized);
    if (spectrum.length < 2) return -1;
    const dcComponent = Math.max(spectrum[0], 0.001);
    const normalizedSpectrum = spectrum.map(s => s / dcComponent);
    const gradient = [];
    for (let i = 0; i < normalizedSpectrum.length - 1; i++) {
        gradient.push(normalizedSpectrum[i + 1] - normalizedSpectrum[i]);
    }
    let arcLength = 0;
    const deltaOmega = 1;
    const omega_c = frequencies[frequencies.length - 1];
    const omega_c_inv = omega_c > 0 ? 1 / omega_c : 1;
    for (let i = 0; i < normalizedSpectrum.length - 1; i++) {
        const arcComponent = Math.sqrt(Math.pow(omega_c_inv, 2) + Math.pow(gradient[i], 2));
        arcLength += arcComponent * deltaOmega;
    }
    return -arcLength;
  }

  function calculateCombinedPenalty(dtwCost, sparcValue) {
    const DTW_THRESHOLD = 10;
    const SPARC_THRESHOLD = -1.2;
    const dtwExcess = Math.max(0, dtwCost - DTW_THRESHOLD);
    const sparcExcess = Math.max(0, SPARC_THRESHOLD - sparcValue);
    const base = 0.2;
    return (base + dtwExcess) * (base + sparcExcess);
  }

  function calculateDTWAndSPARCPenalty() {
    ...
    let idealFlattened = idealPathPoints.map(pt => Math.hypot(pt.x - center.x, pt.y - center.y) - shapeRadius);
    let cursorFlattened = drawnPath.map(pt => Math.hypot(pt.x - center.x, pt.y - center.y) - shapeRadius);

    const DTW_TARGET_LENGTH = 1000;
    let idealResampled = resample1DArray(idealFlattened, DTW_TARGET_LENGTH);
    let cursorResampled = resample1DArray(cursorFlattened, DTW_TARGET_LENGTH);

    let dtwCost = computeDTWDistance(idealResampled, cursorResampled);
    const speedProfile = calculateSpeedProfile(drawnPath);
    const sparcValue = calculateSPARC(speedProfile);
    const combinedScore = calculateCombinedPenalty(dtwCost, sparcValue);
    
    //const PENALTY_SCALE = 0.00001;
    const PENALTY_SCALE = 0.01;
    //const finalPenalty = combinedScore * PENALTY_SCALE;
    const finalPenalty = dtwCost * PENALTY_SCALE;
    console.log(`Trial Smoothness Analysis -> DTW: ${dtwCost.toFixed(2)}, SPARC: ${sparcValue.toFixed(2)}, Deduction: ${finalPenalty.toFixed(4)}`);
    return finalPenalty;
  }
  ...
  function processOneDelta(dx, dy, now, sourceTag) {
    ...
    if (!reachedTarget && reachedTargetPoint(penPos.x, penPos.y)) {
      ...
      const penalty = calculateDTWAndSPARCPenalty();
  ...
  function attachCanvasListenersTablet() {
    ...
    canvas.addEventListener('pointermove', (e) => {
      if (e.pointerType !== 'pen' || !trialStarted || !tabletActive || !drawing) return;
      ...
      let dx = e.clientX - lastPenPos.x;
      let dy = e.clientY - lastPenPos.y;
      lastPenPos = { x: e.clientX, y: e.clientY };
    ...
    canvas.addEventListener('pointerup', (e) => {
      if (e.pointerType !== 'pen') return;
      tabletActive = false; lastPenPos = null;
      try { canvas.releasePointerCapture(e.pointerId); } catch(err) {}
      drawAmoeba();
    });
  ...
  function displayBlockResultsGraph() {
    ...
    let maxTrialsInBlock = BLOCKS[currentBlockIdx].maxTrials;
